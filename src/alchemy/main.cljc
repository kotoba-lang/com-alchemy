(ns alchemy.main
  "Kotodama WASM entrypoint for the Alchemy clean-room actor (L4) — Clojure port.
  ...
  facts (`alchemy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "alchemy")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"alchemy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "alchemy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :alchemy-compat/ready)
